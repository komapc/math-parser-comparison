...
in all 12 size×run measurements, a tie — and the top-down family that lost
its AVX2 candidate scan in the port is slower in Rust, unevenly: pointer
`multipass` itself is closest at ~1.2×, while the arena/direct forms that
actually used the scan (`multipass-arena`, `direct-mp`, `multipass-bfs`)
are 1.4–1.6× behind C++. On the structured shapes the fold beats
`ast-arena` by 9–39 % on towerchain, sumchain and nestchain in every run
and ties it on powchain (−8…+5 %); `direct-reverse` ties `direct-rd` on
the random corpus, is only narrowly ahead on powchain (+2–4 %), and
10–22 % ahead on towerchain, sumchain and nestchain. Full cross-language
tables: [FINDINGS.md](../FINDINGS.md), [docs/one-pager.md](../docs/one-pager.md).
