...
- **Against its own family it is strictly better everywhere** — 1.3–1.5× faster
  than the buffered version of itself, which is in turn 1.7–3.8× faster than
  the top-down divide-and-conquer variants on their C++ worst-case shapes
  (2.3–5× for the fused form itself; see the scoreboard below for the other
...
- **In Haskell the tree-building form loses** to the pointer-AST classics by
  ...
  random corpus and powchain, but loses towerchain, sumchain and nestchain to
  recursive descent by 2–25 %.
...
| random corpus (n=10000) | ahead, narrowly (+3…+5 % vs `ast-arena`) | tie (-1…0 % vs `ast-arena`) | ahead, narrowly (+4…+8 % vs `ast-sy`) | loses (1.7× slower than `ast-pratt`) |
| powchain | ahead, narrowly (+4…+16 % vs `ast-arena`) | tie (-8…+5 % vs `ast-arena`) | loses (-10…-6 % vs `ast-rd`) | loses (1.5–1.6× slower than `ast-pratt`) |
| towerchain | **best** (+14 % vs `ast-arena`) | **best** (+9…+15 % vs `ast-arena`) | tie (-2…+2 % vs `ast-rd`) | loses (1.5–1.7× slower than `ast-pratt`) |
| sumchain | **best** (+20…+22 % vs `ast-arena`) | **best** (+8…+22 % vs `ast-arena`) | ahead, narrowly (+2…+3 % vs `ast-pratt`) | loses (1.5–1.8× slower than `ast-rd`) |
| nestchain | **best** (+22…+35 % vs `ast-arena`) | **best** (+22…+39 % vs `ast-arena`) | loses (-1 % vs `ast-sy`) | loses (1.7–1.9× slower than `ast-pratt`) |
...
| sumchain | ahead, narrowly (+2…+29 % vs `direct-sy`) | **best** (+15 % vs `direct-rd`) | **best** (+14…+16 % vs `direct-rd`) | loses (-7…-2 % vs `direct-rd`) |
...
| towerchain | **best** (+53 % vs `direct-mp`) | **best** (+53…+57 % vs `direct-mp`) | **best** (+37 % vs `direct-mp`) | **best** (+39…+45 % vs `direct-mp`) |
...
- It is not faster than a lexer-free evaluator: `direct-scannerless`, recursive
  ...
  Rust and Python by 20–35 %. That is the cost of having a shared lexer at
  all and every real strategy in the table pays it; it is the control, not a
  rival.
