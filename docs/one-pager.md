...
the Rust column was re-run three times after the fold moved to safe Rust).
...
- **Against the classic parsers it is a peer in C++, Rust and Python**
  ...
  run to run), ahead on three of Rust's four structured shapes — clearly
  only on nestchain — and a tie on powchain; the fastest tree builder and the fastest
...
|---|---|---|---|---|
| random corpus (n=1000) | ahead, narrowly (+3…+4 % vs `ast-arena`) | tie (-5…+5 % vs `ast-arena`) | tie (0…+2 % vs `ast-rd`) | loses (1.2–1.3× slower than `ast-rd`) |
| random corpus (n=10000) | ahead, narrowly (+1…+4 % vs `ast-arena`) | tie (-5…+2 % vs `ast-arena`) | **best** (+5…+6 % vs `ast-sy`) | loses (1.5–1.6× slower than `ast-pratt`) |
| powchain | **best** (+10…+16 % vs `ast-arena`) | tie (-1…+2 % vs `ast-arena`) | loses (-4…-4 % vs `ast-rd`) | loses (1.4–1.5× slower than `ast-pratt`) |
| towerchain | tie (-1…+17 % vs `ast-arena`) | ahead, narrowly (+3…+8 % vs `ast-arena`) | tie (0…+2 % vs `ast-rd`) | loses (1.3–1.4× slower than `ast-pratt`) |
| sumchain | **best** (+19…+21 % vs `ast-arena`) | ahead, narrowly (+2…+11 % vs `ast-arena`) | ahead, narrowly (+1…+2 % vs `ast-rd`) | loses (1.4–1.6× slower than `ast-rd`) |
| nestchain | **best** (+16…+29 % vs `ast-arena`) | **best** (+19…+30 % vs `ast-arena`) | tie (0…0 % vs `ast-sy`) | loses (1.7–1.8× slower than `ast-pratt`) |

### No-tree form `direct-reverse` vs the best classic no-tree evaluator

| input | C++ | Rust | Python | Haskell |
|---|---|---|---|---|
| random corpus (n=1000) | tie (0…+1 % vs `direct-rd`) | tie (-8…0 % vs `direct-rd`) | **best** (+8…+9 % vs `direct-rd`) | tie (-6…+10 % vs `direct-rd`) |
| random corpus (n=10000) | tie (0…+1 % vs `direct-rd`) | tie (-8…0 % vs `direct-rd`) | **best** (+6…+8 % vs `direct-rd`) | ahead, narrowly (+4…+9 % vs `direct-rd`) |
| powchain | ahead, narrowly (+2…+9 % vs `direct-sy`) | ahead, narrowly (+5…+6 % vs `direct-rd`) | ahead, narrowly (+3…+3 % vs `direct-rd`) | **best** (+21…+26 % vs `direct-rd`) |
| towerchain | tie (-1…+1 % vs `direct-sy`) | **best** (+9…+11 % vs `direct-rd`) | **best** (+17…+19 % vs `direct-rd`) | ahead, narrowly (+5…+7 % vs `direct-rd`) |
| sumchain | tie (-4…+32 % vs `direct-sy` — this shape is volatile on this runner across batches, see [FINDINGS.md](../FINDINGS.md)) | **best** (+13…+18 % vs `direct-rd`) | **best** (+11…+13 % vs `direct-rd`) | loses (-3…-1 % vs `direct-rd`) |
| nestchain | loses (-24…-14 % vs `direct-sy`) | **best** (+17…+21 % vs `direct-sy`) | ahead, narrowly (+4…+4 % vs `direct-sy`) | loses (-20…-19 % vs `direct-rd`) |

### `multipass-reverse-fold` vs the best of its own top-down family

| input | C++ | Rust | Python | Haskell |
|---|---|---|---|---|
| random corpus (n=1000) | **best** (+31…+34 % vs `direct-mp`) | **best** (+56…+60 % vs `direct-mp`) | **best** (+55…+56 % vs `direct-mp`) | **best** (+28…+32 % vs `multipass`) |
| random corpus (n=10000) | **best** (+37…+38 % vs `direct-mp`) | **best** (+63…+65 % vs `direct-mp`) | **best** (+59 % vs `direct-mp`) | **best** (+40…+42 % vs `direct-mp`) |
| powchain | **best** (+51…+54 % vs `direct-mp`) | **best** (+72…+74 % vs `direct-mp`) | **best** (+47…+49 % vs `direct-mp`) | **best** (+35…+42 % vs `multipass`) |
| towerchain | **best** (+50…+54 % vs `direct-mp`) | **best** (+54…+56 % vs `direct-mp`) | **best** (+33…+34 % vs `direct-mp`) | **best** (+49…+54 % vs `direct-mp`) |
| sumchain | **best** (+37…+45 % vs `direct-mp` — its closest family rival here, `multipass-arena`, has a bimodal runtime on this shape; see the note below) | **best** (+49…+57 % vs `direct-mp`) | **best** (+33…+34 % vs `direct-mp`) | **best** (+10…+12 % vs `multipass`) |
| nestchain | **best** (+49…+52 % vs `direct-mp`) | **best** (+28…+33 % vs `direct-mp`) | **best** (+45…+46 % vs `direct-mp`) | **best** (+31…+36 % vs `multipass`) |
...
Rust column: 35906739767, 35906748758, 35906757643 (2026-09-23).*
